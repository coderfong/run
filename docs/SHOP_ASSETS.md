...
The hero is CROPPED, not rescaled: `SCENE.visibleHeight` (1014 of 1146) cuts the
dead bottom off the counter skirt so the purple stripe becomes the bottom trim.
Every frame above is still authored against the full 1146, so nothing moved —
but any new full-scene layer must be sized with `sceneBox`, not `absoluteFill`,
or it will shrink to the shorter box and drift out of register with the crew.

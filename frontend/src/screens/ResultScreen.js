...
export default function ResultScreen({ navigation, route }) {
  ...
  // Rank progression is a real screen pushed over this one, so leaving for it
  // IS the end of the celebration. It used to navigate away mid-payoff, and
  // its Done came back to a claim sequence still parked on the payoff phase —
  // a frozen map with nothing left to press. The stage is closed on the way
  // out; the outro overlays wait until the runner is back, since a popup
  // fired now would open over (or behind) the screen they just went to.
            ...
            {canPlace && (
              <>
                ...
                <View style={styles.claimFooter}>
                  {!!preview?.energy_cost && (
                    <View style={styles.claimCost}>
                      <Text style={type.caption}>{`Costs ${preview.energy_cost} energy`}</Text>
                    </View>
                  )}
                  {energyStatus && (
                    <EnergyMeter
                ...
                {/* "Looks good? Claim it." lights THIS button and lets the
                    press through to it. */}
                <View {...claimButtonTarget} collapsable={false} style={styles.claimButtonWrap}>
                  <ToonButton
